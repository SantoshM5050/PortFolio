          ...
          {skills.map((s, i) => (
            <div key={i} className="p-4 rounded-2xl bg-slate-950/90 border border-emerald-500/40 space-y-2 text-xs font-tech">
              <div className="flex justify-between items-center text-white font-orbitron">
                <span>{s.name}</span>
                <span className="text-[10px] text-cyan-400 font-bold px-2 py-0.5 rounded bg-emerald-950 border border-emerald-500/40">{s.warp}</span>
              </div>
              <div className="w-full h-2 rounded-full bg-black border border-emerald-500/40 overflow-hidden">
                <div className="h-full bg-gradient-to-r from-emerald-400 via-teal-400 to-cyan-500 rounded-full shadow-[0_0_12px_rgba(0,255,102,0.8)] w-full" />
              </div>
            </div>
          ))}
        </div>
